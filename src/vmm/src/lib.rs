...
impl Vmm {
    ...
    pub fn resume_vm(&mut self) -> Result<(), VmmError> {
        ...
        if vm.can_resume() {
            return vm.resume().map_err(|err| {
                println!("Failed to resume VM: {}", err);
                VmmError::Vm(VmError::ResumeVm)
            });
        }

        if !vm.can_start() {
            println!("VM can't start!");
            return Err(VmmError::Vm(VmError::StartVm));
        }

        println!("Starting VM...");

        vm.start().map_err(|err| {
            println!("Failed to start VM: {}", err);
            VmmError::Vm(VmError::StartVm)
        })?;
    ...
    pub fn pause_vm(&mut self) -> Result<(), VmmError> {
        ...
        if !vm.can_pause() {
            println!("VM can't pause!");
            return Err(VmmError::Vm(VmError::PauseVm));
        }

        println!("Pausing VM...");

        vm.pause().map_err(|err| {
            println!("Failed to pause VM: {}", err);
            VmmError::Vm(VmError::PauseVm)
        })?;
